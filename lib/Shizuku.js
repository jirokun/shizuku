...
var Shizuku = function () {
  ...
  _createClass(Shizuku, [{
    ...
    key: 'findOutputComponents',
    value: function findOutputComponents() {
      var _this6 = this;

      var shizukucomponentMap = this._el.querySelectorAll('.shizuku-component-container');
      var allComponents = Array.prototype.map.call(shizukucomponentMap, function (el) {
        return _this6.getComponent(el);
      });
      return allComponents.filter(function (c) {
        return c instanceof _OutputComponent2.default;
      });
    }
    ...
    key: 'getParentComponentOrderByProcess',
    value: function getParentComponentOrderByProcess(component) {
      function getParentsAndSelf(c) {
        var componentSet = arguments.length <= 1 || arguments[1] === undefined ? new Set() : arguments[1];

        var sources = c.getSourceComponents();
        if (sources.length !== 0) {
          sources.forEach(function (child) {
            return getParentsAndSelf(child, componentSet);
          });
        }
        componentSet.add(c);
        return componentSet;
      }
      function orderByProccess(components) {
        var list = arguments.length <= 1 || arguments[1] === undefined ? [] : arguments[1];

        components.forEach(function (c) {
          // 処理対象じゃないまたは
          // 処理対象かつ一つでも親がlistに入っていなければ後回し
          if (!parentsAndSelf.has(c) || parentsAndSelf.has(c) && c.getSourceComponents().find(function (pc) {
      ...
      // 一度全ての親を取得する
      var parentsAndSelf = getParentsAndSelf(component);
      // その中で開始コンポーネントを取得する
      var firstComponents = Array.from(parentsAndSelf).filter(function (c) {
        return c.getSourceComponents().length === 0;
      });
      // 降順に並べる
      var orderdComponent = orderByProccess(firstComponents);
    ...
    key: 'run',
    value: function run() {
      this.findOutputComponents().forEach(function (c) {
        return c.execute();
      });
    }
